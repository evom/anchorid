<!DOCTYPE html>
<html>
<head>
	<title>Sitemap - AnchorID</title>
	<?php include('includes/head.php'); ?>
</head>
<body>
<!-- Header Starts Here -->
<?php include "includes/header.php";?>
<!-- About Page Starts Here -->
<div class="outer-container">
<div class="container">
	<div class="row about-row-setting">  

		<div class="col-md-12">
			<h1 class="main-heading">Sitemap</h1>
		</div>
	</div>
	<div class="row">
		<div class="col-md-4">
			<div id="section-content" class="sitemaps">
			<ul>
				<li class="sitemap-menu"><a href="index.php" class="menu"><span>Home</span></a></li>
				<li class="sitemap-menu"><a href="how-it-works.php" class="menu"><span>How it Works</span></a></li>
				<li class="sitemap-menu"><a href="about.php" class="menu"><span>About</span></a></li>
				<li class="sitemap-menu sitemap-menu_child"><a href="who-we-are.php">Who we are</a></li>
				<li class="sitemap-menu sitemap-menu_child"><a href="who-uses-anchorid.php">Who uses AnchorID</a></li>
			</ul>
			</div>
		</div>
		<div class="col-md-4">
			<div id="section-content" class="sitemaps">
			<ul>
				<li class="sitemap-menu sitemap-menu_child"><a href="superior-safety.php">Superior Safety</a></li>
				<li class="sitemap-menu sitemap-menu_child"><a href="team.php">Team</a></li>
				<li class="sitemap-menu sitemap-menu_child"><a href="news.php">News </a></li>
				<li class="sitemap-menu sitemap-menu_child"><a href="careers.php">Careers</a></li>
				<li class="sitemap-menu sitemap-menu_child"><a href="contact-us.php">Contact us</a></li>
			</ul>
			</div>
		</div>
		<div class="col-md-4">
			<div id="section-content" class="sitemaps">
			<ul>
				<li class="sitemap-menu"><a target="_blank" href="http://developers.anchorid.com/"><span>Developer</span></a></li>
				<li class="sitemap-menu sitemap-menu_child"><a href="http://developers.anchorid.com">Register</a></li>
				<li class="sitemap-menu sitemap-menu_child"><a target="_blank" href="http://developers.anchorid.com/">Sign in</a></li>
				<li class="sitemap-menu sitemap-menu_child"><a target="_blank" href="http://developers.anchorid.com/">Documentation</a></li>
				<li class="sitemap-menu"><a href="support.php" class="menu"><span>Support</span></a></li>	
			</ul>
				<div class="text-right"><a class="sitemap-web" href="http://dannyglix.com">Webmaster</a></div>
			</div>
		</div>
	</div>
</div>
</div>

<?php include "includes/footer.php"; ?>
</body>
</html>
